...
std::unique_ptr<SessionHandler> SessionHandler::createSession(const UserCredentials& credentials)
{
    ...
    try
    {
        auto& persistentData = PersistenceHandler::instance();
            ...
            )
        {
            ...
            if (credentials.roles[0] == "Borrower") return std::make_unique<BorrowerSession>(credentials);
            if (credentials.roles[0] == "Librarian") return std::make_unique<LibrarianSession>(credentials);
            if (credentials.roles[0] == "Administrator") return std::make_unique<AdministratorSession>(credentials);
            if (credentials.roles[0] == "Management") return std::make_unique<ManagementSession>(credentials);
    ...
    catch (const PersistenceHandler::NoSuchUser&) {}  // Catch and ignore this anticipated condition
