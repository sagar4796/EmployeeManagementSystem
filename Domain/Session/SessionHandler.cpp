    ...
    try
    {
        ...
        )
      {
        // 2) If authenticated user is authorized for the selected role, create a session specific for that role
         if( credentials.roles[0] == "Admin"      ) return std::make_unique<Domain::Session::AdminSession>     ( credentials );
        if( credentials.roles[0] == "Manager"     ) return std::make_unique<Domain::Session::ManagerSession>   ( credentials );
        if( credentials.roles[0] == "Employee" ) return std::make_unique<Domain::Session::EmployeeSession>     ( credentials );
